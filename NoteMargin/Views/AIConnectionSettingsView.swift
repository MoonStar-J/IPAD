...
struct AIConnectionSettingsView: View {
    ...
    @ObservedObject private var connection = AIConnectionStore.shared
    ...
    var body: some View {
        NavigationStack {
            ...
            .onChange(of: provider) { _, next in
                key = ""; saved = false
                model = UserDefaults.standard.string(forKey: "ai.model.\(next.rawValue)") ?? next.defaultModel
            }
