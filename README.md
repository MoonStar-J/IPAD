...
**현재 상태:** 기본 기능 소스 작성, Core 검증 12개 통과, Swift 문법·프로젝트 설정·아이콘 규격 검사 완료. 작업한 Mac에는 Xcode가 없어 **iPad 앱 전체의 SDK 타입 검사, 빌드, 시뮬레이터 실행, 실제 설치는 아직 수행하지 않았습니다.** 서명된 `.ipa`는 포함하지 않습니다.
...
| 입력 | Apple Pencil 우선, 손가락 필기 설정, 확대·이동, 화면에 용지 맞추기 |
| 용지 | 무지·줄·격자·도트, 페이지 추가·복제·삭제·순서 변경 |
| PDF | 파일 앱에서 가져오기, 다른 앱에서 PDF 열기, 페이지별 필기 |
